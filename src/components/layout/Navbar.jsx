...
export default function Navbar() {
  ...
  // على الصفحة الرئيسية فقط: روابط بيضاء فوق الـ Hero الداكن
  // على باقي الصفحات: روابط داكنة لأن الخلفية فاتحة
  ...
  const handleLogout = () => {
    authLogout(); setUserMenuOpen(false); setMobileOpen(false);
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled || !isHome
          ? 'bg-white/95 dark:bg-stone-950/95 backdrop-blur-lg shadow-lg shadow-stone-200/40 dark:shadow-stone-900/40 border-b border-stone-100/50 dark:border-stone-800/50'
          : 'bg-transparent'
      }`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 lg:h-20">
            <Link to="/" className="flex items-center gap-2 flex-shrink-0">
              ...
              <span className={`font-display font-bold text-xl transition-colors duration-200 ${darkText ? 'text-stone-900 dark:text-white' : 'text-white'}`}>
            ...
            <div className="hidden lg:flex items-center gap-1">
              {navLinks.map(link => (
                <NavLink key={link.to} to={link.to} end={link.end}
                  className={({ isActive }) => {
                    if (darkText) {
                      return `px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${isActive ? 'text-orange-600 bg-orange-50 dark:bg-orange-950/20 dark:text-orange-400' : 'text-stone-600 hover:text-orange-600 hover:bg-orange-50 dark:text-stone-400 dark:hover:text-orange-400 dark:hover:bg-orange-950/20'}`;
                    }
                    return `px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${isActive ? 'text-white bg-white/10' : 'text-white/80 hover:text-white hover:bg-white/10'}`;
                  }}
                >{link.label}</NavLink>
              ))}
            </div>
            ...
            <div className="flex items-center gap-1">
              {!hideSearch && (
                <button onClick={() => setSearchOpen(true)} className={`p-2 rounded-xl transition-all duration-200 ${darkText ? 'text-stone-600 hover:text-orange-600 hover:bg-orange-50 dark:text-stone-400 dark:hover:text-orange-400 dark:hover:bg-orange-950/20' : 'text-white/80 hover:text-white hover:bg-white/10'}`}><HiOutlineSearch className="w-5 h-5" /></button>
              )}
              <button onClick={toggleTheme} className={`p-2 rounded-xl transition-all duration-200 ${darkText ? 'text-stone-600 hover:text-orange-600 hover:bg-orange-50 dark:text-stone-400 dark:hover:text-orange-400 dark:hover:bg-orange-950/20' : 'text-white/80 hover:text-white hover:bg-white/10'}`}>
              ...
              {isSeller && isAuthenticated && (
                <Link to="/seller/dashboard" className={`hidden sm:flex p-2 rounded-xl relative group transition-all duration-200 ${darkText ? 'text-stone-600 hover:text-orange-600 hover:bg-orange-50 dark:text-stone-400 dark:hover:text-orange-400 dark:hover:bg-orange-950/20' : 'text-white/80 hover:text-white hover:bg-white/10'}`}>
              ...
              {isAuthenticated ? (
                <div className="relative hidden sm:block" data-user-menu>
                  <button onClick={() => setUserMenuOpen(v => !v)} className={`flex items-center gap-2 pl-1 pr-3 py-1 rounded-xl transition-all ${darkText ? 'hover:bg-stone-100 dark:hover:bg-stone-800' : 'hover:bg-white/10'}`}>
                    <div className="w-7 h-7 rounded-full bg-gradient-to-br from-orange-500 to-teal-500 flex items-center justify-center text-white text-xs font-bold border-2 border-orange-400">{userData?.name?.charAt(0).toUpperCase()}</div>
                    <span className={`text-sm font-medium max-w-[80px] truncate ${darkText ? 'text-stone-700 dark:text-stone-300' : 'text-white/90'}`}>{userData?.name?.split(' ')[0]}</span>
                  </button>
                ...
                <Link to="/login" className={`hidden sm:flex p-2 rounded-xl transition-all duration-200 ${darkText ? 'text-stone-600 hover:text-orange-600 hover:bg-orange-50 dark:text-stone-400 dark:hover:text-orange-400 dark:hover:bg-orange-950/20' : 'text-white/80 hover:text-white hover:bg-white/10'}`}><HiOutlineUser className="w-5 h-5" /></Link>
              )}

              <button onClick={() => setMobileOpen(true)} className={`lg:hidden p-2 rounded-xl ml-1 transition-colors duration-200 ${darkText ? 'text-stone-600 hover:text-orange-600 hover:bg-orange-50 dark:text-stone-400 dark:hover:text-orange-400 dark:hover:bg-orange-950/20' : 'text-white/80 hover:text-white hover:bg-white/10'}`}><HiOutlineMenu className="w-6 h-6" /></button>
            </div>
          </div>
        </div>
      </nav>
              ...
              <div className="flex items-center justify-between p-6 border-b border-stone-100 dark:border-stone-800">
                {isAuthenticated ? (
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-gradient-to-br from-orange-500 to-teal-500 flex items-center justify-center text-white text-sm font-bold border-2 border-orange-400">{userData?.name?.charAt(0).toUpperCase()}</div>
                    <div>
